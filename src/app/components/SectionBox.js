...
const SectionBox = ({ text, image, className, imageClass }) => {
  return (
    ...
    >
      <div className="w-10/12 text-[3vw] sm:text-[1vw] flex items-center justify-center xl:text-base">{text}</div>
      <img className={` ${imageClass} w-3/12 sm:w-2/12`} src={`/tech-icons/${image}.png`} />
    </div>
  );
};
